...
void LayerSet::Cull(const sf::FloatRect& bounds)
{
	...
	m_visiblePatchStart.x = static_cast<int>(std::floor((bounds.left / m_tileSize.x) / m_patchSize) - 2000);
	m_visiblePatchStart.y = static_cast<int>(std::floor((bounds.top / m_tileSize.y) / m_patchSize) - 2000);
	if(m_visiblePatchStart.x < 0) m_visiblePatchStart.x = 0;
	if(m_visiblePatchStart.y < 0) m_visiblePatchStart.y = 0;

	m_visiblePatchEnd.x = static_cast<int>(std::ceil((bounds.width / m_tileSize.x) / m_patchSize) + 2000);
	m_visiblePatchEnd.y = static_cast<int>(std::ceil((bounds.height / m_tileSize.y) / m_patchSize) + 2000);
