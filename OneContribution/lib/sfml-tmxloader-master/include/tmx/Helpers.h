...
namespace Helpers
{
	namespace Vectors
	{
		static inline sf::Vector2f randomVector(sf::Vector2f min, sf::Vector2f max)
		{
			std::random_device rand_dev;
			std::mt19937 generator(rand_dev());
			std::uniform_real_distribution<float> randomX(min.x, max.x);
			std::uniform_real_distribution<float> randomY(min.y, max.y);

			float rY = randomY(generator);
			float rX = randomX(generator);
